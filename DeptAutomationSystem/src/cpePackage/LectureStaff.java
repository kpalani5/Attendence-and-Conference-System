		...
		b1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try 
				{
					...
					PreparedStatement stmt=con.getConnection().prepareStatement("insert into lecture values (?,?,?,?,?,?,?,?,?)");
					stmt.setLong(1, d.getSno());
					stmt.setString(2, d.getName());
					stmt.setString(3, d.getDate());
					stmt.setString(4, d.getTopic());
					stmt.setString(5, d.getDuration());
					stmt.setString(6, t2.getText());
					stmt.setString(7, t1.getText());
					stmt.setString(8, d.getInst());
					stmt.setString(9, d.getPlace());
					stmt.execute();
				}
				catch(Exception e1)
				{
...
public class LectureStaff extends JPanel {

	/**
	 * Create the panel.
	 */
	public LectureStaff(final JFrame F,final CpeData d) {
		setLayout(null);
		
		JLabel lblAttendingStaff = new JLabel("GUEST STAFF");
		...
		btnAddStaff.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				panel.add(new subPanel1().create(d));
				panel.revalidate();
				panel.repaint();
			}
		});
