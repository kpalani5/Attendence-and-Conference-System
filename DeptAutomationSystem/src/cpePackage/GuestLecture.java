...
public class GuestLecture extends JPanel {
	 ...
	 * Create the panel.
	 */
	public GuestLecture(final JFrame F,final long sn) {
		setLayout(null);
		
		JLabel lblAttending = new JLabel("GUEST LECTURE");
		...
		JLabel lblConferenceType = new JLabel("TOPIC");
		panel.add(lblConferenceType, "2, 14, 1, 2");
		
		textField_6 = new JTextField();
		panel.add(textField_6, "10, 14, fill, default");
		...
		btnNext.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				d.setTopic(textField_6.getText());
				
				if(chk==1)
				{
				Container p=F.getContentPane();
				p.removeAll();
				p.add(new LectureStaff(F,d));
				p.revalidate();
				p.repaint();
				}
			}
