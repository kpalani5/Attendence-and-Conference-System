...
public class PeriodwiseAttendance extends JPanel {
	 ...
	 * Create the panel.
	 */
	public PeriodwiseAttendance(final JFrame F) {
		setLayout(null);
		
		JLabel lblPeriodwiseAttendance = new JLabel("PERIODWISE ATTENDANCE");
		...
		btnGenerate.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try
				{
					...
					HSSFSheet sheet = workbook.createSheet("ATTENDANCE FOR CSE "+sc+" SEMESTER "+textField.getText()+" PERIOD "+textField_2.getText());
					HSSFRow row = sheet.createRow(rct++);
					cct=0;
					HSSFCell cel1=row.createCell(cct++);
					cel1.setCellValue("ATTENDANCE FOR CSE "+sc+" SEMESTER "+textField.getText()+" PERIOD "+textField_2.getText());
			        ...
			        Paragraph paragraph = new Paragraph("ATTENDANCE FOR CSE "+sc+" SEMESTER "+textField.getText()+" PERIOD "+textField_2.getText());
					...
					while(rs1.next())
					{
						...
						while(rs3.next())
						{
						stmt2=con.getConnection().prepareStatement("select subcode,hours_attended,total_hours from attended where regno=? and period=? and subcode=?");
						...
						while(rs2.next())
						{
							...
							pdfTable.addCell(rs2.getString(2)+" / "+rs2.getString(3));
							
							cell = row.createCell(cct++);
							cell.setCellValue(rs2.getString(2)+" / "+rs2.getString(3));
	...
	public static void main(String args[])
	{
		...
		pane.add(new PeriodwiseAttendance(F));
		pane.revalidate();
		pane.repaint();
	}
