...
public class SubjectwiseAttendance extends JPanel {
	 ...
	 * Create the panel.
	 */
	public SubjectwiseAttendance(final JFrame F) {
		setLayout(null);
		
		JLabel lblPeriodwiseAttendance = new JLabel("SUBJECTWISE ATTENDANCE");
		...
		btnGenerate.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try
				{
					...
					while(pd<=4)
					{
						pdfTable.addCell(new PdfPCell(new Phrase("PERIOD "+new Integer(pd).toString())));
						pd++;
					}
					
					cell = row.createCell(cct++);
					cell.setCellValue("PERIOD 1");
					cell = row.createCell(cct++);
					cell.setCellValue("PERIOD 2");
					cell = row.createCell(cct++);
					cell.setCellValue("PERIOD 3");
					cell = row.createCell(cct++);
					cell.setCellValue("PERIOD 4");
	...
	public static void main(String args[])
	{
		...
		pane.add(new SubjectwiseAttendance(F));
		pane.revalidate();
		pane.repaint();
	}
